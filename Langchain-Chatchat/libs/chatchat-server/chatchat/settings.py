...
class KBSettings(BaseFileSettings):
    ...
    """知识库匹配相关度阈值。LangChain VectorStoreRetriever 要求 0~1；取 1.0 最宽松（接近不筛），越小越严。"""
    ...
    PDF_CLEANUP_ENABLE: bool = True
    """是否对 PDF 文本做基础清洗（去页眉页脚/页码等重复噪声）。"""

    PDF_HEADER_FOOTER_MAX_LINES: int = 3
    """每页最多采样多少行作为页眉/页脚候选（仅用于重复行检测）。"""

    PDF_HEADER_FOOTER_MIN_REPEAT: float = 0.6
    """重复行判定阈值（0-1），出现频率 >= 该比例的候选行会被作为页眉/页脚噪声移除。"""
...
class PromptSettings(BaseFileSettings):
    ...
    rag: dict = {
        ...
        "building_fm_esg": (
            "你是建筑能源管理、设施管理(FM)与 ESG 报告领域的智能运营顾问。\n"
            "请结合【已知信息】与用户在【问题】中给出的能耗/设备/空间/视觉识别等指标，"
            "输出可落地的优化建议：每条建议尽量包含——问题描述、建议动作、预期节能或提效效果（可做区间估算）、优先级（高/中/低）。\n"
            "若信息不足，请明确列出需要补充的数据项。\n"
            "回答使用中文，条理清晰。\n\n"
            "【已知信息】\n{{context}}\n\n"
            "【问题与现场指标】\n{{question}}\n"
        ...
        "smart_ops": (
            "你是建筑智能运营与节能顾问。请结合【已知信息】与用户给出的现场数据（可含：空间编号、视觉无人时长、"
            "空调/照明状态、能耗读数），输出可执行建议：问题描述、建议动作、预计节电区间、优先级、需补充数据。\n"
            "中文输出。\n\n"
            "【已知信息】\n{{context}}\n\n"
            "【现场数据与问题】\n{{question}}\n"
        ),
        "fm_maintenance": (
            "你是设施管理(FM)与设备运维专家。结合【已知信息】与用户描述的异常，给出可能原因、排查步骤、安全注意与工单建议。\n"
            "中文分条。\n\n"
            "【已知信息】\n{{context}}\n\n"
            "【故障与问题描述】\n{{question}}\n"
        ),
        "esg_compliance": (
            "你是建筑与不动产 ESG 顾问。结合【已知信息】回答用户关于指标、披露与合规的问题，给出可执行建议。\n"
            "区分知识库依据与常识推断。\n\n"
            "【已知信息】\n{{context}}\n\n"
            "【问题】\n{{question}}\n"
        ),
        "system_qa": (
            "你是建筑能源、FM 与 ESG 领域专业助手。严格依据【已知信息】作答；不足时请说明并列出需补充信息。\n"
            "中文，先结论后要点。\n\n"
            "【已知信息】\n{{context}}\n\n"
            "【问题】\n{{question}}\n"
        ),
