...
class OpenAIBaseInput(BaseModel):
    # Pydantic v2：必须用 model_config，旧的 class Config(extra=...) 不生效，
    # 否则 JSON 里的 score_threshold / prompt_name 等会被丢弃，知识库对话会回退到 kb_settings 默认阈值（如 2.0）并触发 LangChain 校验错误。
