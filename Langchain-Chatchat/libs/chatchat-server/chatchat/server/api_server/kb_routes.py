...
def _clamp_score_threshold_for_lc(raw) -> float:
    """
    LangChain VectorStoreRetriever 要求 search_kwargs['score_threshold'] 为 0~1 的 float。
    历史配置 / kb_settings 可能仍为 2.0（「不筛选」语义），此处钳制到 1.0。
    """
    try:
        v = float(raw)
...
async def kb_chat_endpoint(
    ...
    st_raw = extra.get("score_threshold", Settings.kb_settings.SCORE_THRESHOLD)
    ret = await kb_chat(
        query=body.messages[-1]["content"],
        mode=mode,
        kb_name=param,
        top_k=extra.get("top_k", Settings.kb_settings.VECTOR_SEARCH_TOP_K),
        score_threshold=_clamp_score_threshold_for_lc(st_raw),
