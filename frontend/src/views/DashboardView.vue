...
function openDataScreen() {
  const r = router.resolve({ path: '/screen' })
...
<template>
  <div class="dash-container">
    ...
    <template v-else>
    <div v-if="anomalySerious || anomalyWarn" class="alert-stack">
      <el-alert
    ...
    <el-dialog
      v-model="incidentTipVisible"
      title="待办提醒"
      width="400px"
      align-center
      :close-on-click-modal="true"
      @close="dismissIncidentTip(false)"
    >
      <p class="incident-tip-body">
        当前有 <strong class="num-font">{{ pendingIncidents }}</strong> 条待处理运维工单，可前往工单页面查看与处理。
      </p>
      <template #footer>
        <el-button @click="dismissIncidentTip(false)">稍后</el-button>
        <el-button type="primary" @click="dismissIncidentTip(true)">查看工单</el-button>
      </template>
    </el-dialog>
    ...
    <header class="dash-header-mini dash-header-single">
      ...
      <div class="dash-action-group">
        <span v-if="lastUpdated" class="dash-update-time">
          {{ REFRESH_MS / 1000 }}s 刷新 · {{ lastUpdated.toLocaleTimeString('zh-CN') }}
        </span>
        <el-button :icon="Monitor" @click="openDataScreen">数据大屏</el-button>
    ...
    <el-row :gutter="[12, 12]" class="dash-chart-row">
      <el-col :xs="24" :lg="16">
        <div class="chart-surface">
          <div class="chart-surface-title">
            {{ lineChartHeading }}
            <span class="chart-surface-sub">{{ buildingIdForSeries || '无建筑' }} · 饼图扇区可切换指标</span>
          </div>
      ...
      <el-col :xs="24" :lg="8">
        <div class="chart-surface">
          <div class="chart-surface-title">
            能源构成（累计）
            <span class="chart-surface-sub">点击扇区联动趋势</span>
          </div>
          <AppChart
            v-if="pieOption"
    ...
    <div class="chart-surface dash-benchmark-card">
      <div class="chart-surface-title">
        建筑能效对标 TOP8
        <span class="chart-surface-sub">柱状</span>
      </div>
