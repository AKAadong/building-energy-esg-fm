...
/** 能力入口：扁平化为顶栏快捷标签 */
const flatNavLinks = computed(() => {
  const out = []
  for (const block of MODULE_NAV) {
    for (const ln of block.links) {
      out.push({ ...ln, group: block.key })
    }
  }
  return out
})

/** 驾驶舱首屏 5 项：市电 / 用水 / 光伏 / 异常占比 / 待办（同比环比占位） */
...
<template>
  <div class="dash-container">
    <div v-if="loading" class="dash-skeleton-wrap" aria-busy="true">
      <el-skeleton animated>
        <template #template>
          ...
          <div class="sk-tags">
            <el-skeleton-item v-for="i in 8" :key="i" variant="rect" class="sk-tag-cell" />
          </div>
    ...
    <div class="dash-nav-tags dash-nav-pills" aria-label="能力入口">
      <el-button
        v-for="ln in flatNavLinks"
        :key="ln.path + ln.label"
        class="dash-nav-tag"
        size="small"
        @click="goPath(ln.path)"
      >
        <el-icon class="dash-nav-tag-ico"><component :is="ln.icon" /></el-icon>
        {{ ln.label }}
      </el-button>
    </div>
