...
import { buildIncidentPayloadFromDamaged, getDamagedApplianceItems } from '@/utils/twinDamagedIncident'
import { useTwinSessionStore } from '@/stores/twinSession'

const router = useRouter()
const twinSession = useTwinSessionStore()
...
const analysisTab = ref('object')
const tipsDialogVisible = ref(false)
const forecastChartRef = ref(null)

function openAnalysisDrawer(tab) {
  analysisTab.value = tab
  analysisDrawerVisible.value = true
  scheduleForecastChartResize()
}

function scheduleForecastChartResize() {
  nextTick(() => {
    requestAnimationFrame(() => {
      forecastChartRef.value?.resize?.()
      setTimeout(() => forecastChartRef.value?.resize?.(), 280)
    })
  })
}

function onAnalysisTabChange(tab) {
  if (tab === 'object') scheduleForecastChartResize()
}
...
/** 从会话恢复时无 File 对象，用此展示文件名 */
const restoredPreviewName = ref('')

/** 识别到破损电器后自动建单弹窗 */
const autoIncidentDialogVisible = ref(false)
const autoIncidentInfo = ref(null)
const creatingAutoIncident = ref(false)

function _revokeBlobPreview() {
...
async function persistTwinSession() {
  if (!uploadResult.value) return
  const raw = selectedFile.value ?? fileList.value?.[0]?.raw
  let previewDataUrl = previewUrl.value?.startsWith('data:') ? previewUrl.value : ''
  if (!previewDataUrl && raw instanceof File) {
    try {
      previewDataUrl = await fileToDataUrl(raw)
    } catch {
      previewDataUrl = ''
    }
  }
  const name =
    raw instanceof File ? raw.name : restoredPreviewName.value || twinSession.previewFileName
  twinSession.save({
...
async function restoreTwinSession() {
  if (!twinSession.hydrateFromStorage() || !twinSession.uploadResult) return false
  uploadResult.value = twinSession.uploadResult
  restoredPreviewName.value = twinSession.previewFileName || ''
  if (twinSession.previewDataUrl) {
    _revokeBlobPreview()
    previewUrl.value = twinSession.previewDataUrl
  }
  if (twinSession.objectDims?.length) {
    objectDims.value = twinSession.objectDims.map((row) => ({
      w: Number(row.w),
      h: Number(row.h),
      d: Number(row.d),
    }))
  }
  selectedObjIdx.value = twinSession.selectedObjIdx ?? 0
  if (twinSession.twinBuildingId) twinBuildingId.value = twinSession.twinBuildingId
  if (twinSession.autoIncidentInfo) autoIncidentInfo.value = twinSession.autoIncidentInfo
  await nextTick()
  return true
}
...
async function loadTwinForecastEnergy() {
  ...
  try {
    ...
    if (twinForecastLabels.value?.length) scheduleForecastChartResize()
  }
}
...
const previewFileName = computed(() => {
  ...
  return restoredPreviewName.value
})
...
watch(
  ...
  async ([boxes, tab]) => {
    ...
    if (objectDims.value.length !== boxes.length) {
...
function _buildModelFromDetections() {
  ...
  modelScene.add(dl)
  modelGrid = new THREE.GridHelper(12, 24, 0x1890ff, 0x1e40af)
  ...
  const wallMat = new THREE.MeshStandardMaterial({ color: 0x2563eb, roughness: 0.98 })
...
async function tryAutoCreateIncidentForDamaged(assetHealth) {
  const damaged = getDamagedApplianceItems(assetHealth)
  if (!damaged.length) return

  const payload = buildIncidentPayloadFromDamaged(damaged, twinBuildingId.value)
  if (!payload) return

  creatingAutoIncident.value = true
  try {
    const res = await api.postIncident(payload)
    const item = res?.item ?? res
    const id = item?.id ?? item?.incident_id
    autoIncidentInfo.value = {
      id,
      title: payload.title,
      count: damaged.length,
      items: damaged.map((it) => ({
        name: it.category_zh || it.label,
        level: it.damage_level,
        integrity: it.integrity_score,
      })),
    }
    autoIncidentDialogVisible.value = true
    await persistTwinSession()
  } catch (e) {
    ElMessage.error(e.message ?? '自动创建工单失败')
  } finally {
    creatingAutoIncident.value = false
  }
}

function dismissAutoIncidentDialog() {
  autoIncidentDialogVisible.value = false
}

function goToAutoIncident() {
  const id = autoIncidentInfo.value?.id
  autoIncidentDialogVisible.value = false
  if (id != null) {
    router.push({ path: '/incidents', query: { open: String(id) } })
  } else {
...
async function runUpload() {
  ...
  try {
    ...
    await tryAutoCreateIncidentForDamaged(uploadResult.value?.asset_health)
    await persistTwinSession()
  } catch (e) {
...
async function runUploadAndClose() {
  await runUpload()
  if (uploadResult.value) {
    paramsDrawerVisible.value = false
  }
}

watch(analysisDrawerVisible, (open) => {
  if (open) scheduleForecastChartResize()
})

watch(analysisTab, (tab) => {
  if (analysisDrawerVisible.value && tab === 'object') scheduleForecastChartResize()
})

onMounted(async () => {
  activeTab.value = 'vision'
  await loadBuildingsForTwin()
  const restored = await restoreTwinSession()
  if (restored) {
    await loadOpsSuggestionsTwin()
    if (twinBuildingId.value) await loadTwinForecastEnergy()
  } else {
...
<template>
  <div class="myems-page twin-vision-view twin-immersive">
    <el-dialog
      v-model="autoIncidentDialogVisible"
      class="auto-incident-dialog"
      width="460px"
      align-center
      :show-close="true"
      :close-on-click-modal="true"
      @close="dismissAutoIncidentDialog"
    >
      <template #header>
        <div class="auto-incident-header">
          <div class="auto-incident-icon" aria-hidden="true">
            <el-icon :size="22"><Tickets /></el-icon>
          </div>
          <div class="auto-incident-header-text">
            <div class="auto-incident-header-title">工单已生成</div>
          </div>
        </div>
      </template>

      <div v-if="autoIncidentInfo" class="auto-incident-body">
        <div class="auto-incident-panel">
          <div class="auto-incident-section">
            <div class="auto-incident-section-label">工单摘要</div>
            <div class="auto-incident-section-main">{{ autoIncidentInfo.title }}</div>
            <div v-if="autoIncidentInfo.id" class="auto-incident-section-meta">
              编号 <span class="num-font">#{{ autoIncidentInfo.id }}</span>
            </div>
          </div>

          <div v-if="autoIncidentInfo.items?.length" class="auto-incident-section">
            <div class="auto-incident-section-label">关联设备</div>
            <div class="auto-incident-section-list">
              <div
                v-for="(it, i) in autoIncidentInfo.items"
                :key="i"
                class="auto-incident-device-row"
                :class="{
                  'auto-incident-device-row--attn': it.level === '中度' || it.level === '偏高',
                }"
              >
                <span class="auto-incident-device-name">{{ it.name }}</span>
                <span v-if="it.level" class="auto-incident-device-tag">{{ it.level }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <template #footer>
        <div class="auto-incident-footer">
          <el-button class="auto-incident-btn-later" @click="dismissAutoIncidentDialog">知道了</el-button>
          <el-button type="primary" class="auto-incident-btn-go" @click="goToAutoIncident">
            前往工单
            <el-icon class="auto-incident-btn-ico"><ArrowRight /></el-icon>
          </el-button>
        </div>
      </template>
    </el-dialog>
    ...
    <div class="page-hero">
      ...
      <div class="workspace-toolbar">
        <el-button type="primary" :icon="Setting" @click="paramsDrawerVisible = true">
          识别与上传
        </el-button>
        <el-button :icon="View" :disabled="!previewUrl" @click="previewDialogVisible = true">
          原图预览
        </el-button>
        <el-button
          :icon="DataAnalysis"
          :disabled="!uploadResult"
          @click="openAnalysisDrawer(modelBoxes.length ? 'object' : 'ops')"
        >
          分析结果
        </el-button>
        <el-button text type="primary" @click="tipsDialogVisible = true">操作说明</el-button>
    ...
    <div class="vision-layout">
      ...
      <el-drawer
        ...
        class="twin-drawer"
      >
        <el-card shadow="never" class="ems-card control-panel drawer-inner-card">
            ...
            <div class="control-section">
              ...
              <div class="params-head-row">
                <el-form-item label="置信度 conf" class="form-item-slider basic-param">
                  ...
                  />
                  <p class="field-tip">默认 0.30；拖动或输入数值</p>
                </el-form-item>
      ...
      <el-dialog
        ...
        align-center
        destroy-on-close
        class="twin-preview-dialog"
      >
        <div class="preview-dialog-body">
          <div v-if="previewFileName" class="preview-file-name dialog-file-name">
            <el-icon class="file-ico"><Document /></el-icon>
            {{ previewFileName }}
          </div>
          <div class="preview-stage preview-stage--dialog">
            <el-image v-if="previewUrl" :src="previewUrl" fit="contain" class="preview-img" />
      ...
      <el-dialog v-model="tipsDialogVisible" title="3D 场景操作说明" width="440px" align-center>
        <ul class="preview-tip-list">
          <li>
            <el-icon class="tip-ico"><CircleCheck /></el-icon>
            <span>识别完成后自动生成可交互 3D 场景。</span>
          </li>
          <li>
            <el-icon class="tip-ico"><CircleCheck /></el-icon>
            <span>物体可拖拽至地面与三面墙；标签随模块移动。</span>
          </li>
          <li>
            <el-icon class="tip-ico"><CircleCheck /></el-icon>
            <span>滚轮缩放视角，鼠标拖动旋转观察。</span>
          </li>
        </ul>
        <div class="hero-tips hero-tips--dialog">
      ...
      <el-drawer
        ...
        direction="rtl"
        size="82%"
        class="twin-drawer twin-drawer--wide"
        @opened="scheduleForecastChartResize"
      >
        <el-tabs v-model="analysisTab" class="analysis-tabs" @tab-change="onAnalysisTabChange">
          <el-tab-pane label="物体与预测" name="object" :disabled="!modelBoxes.length">
            <div v-if="uploadResult && modelBoxes.length" class="drawer-tab-body">
        <el-row :gutter="20">
          <el-col :xs="24" :md="9">
          ...
          <el-col :xs="24" :md="15">
            <div v-loading="loadingTwinForecastEnergy" class="twin-forecast-block">
              ...
              <div
                v-if="twinForecastLabels.length && analysisDrawerVisible && analysisTab === 'object'"
                class="twin-forecast-chart-wrap"
              >
                <AppChart
            ...
            <el-empty v-else description="请先完成上传识别" :image-size="64" />
          </el-tab-pane>

          <el-tab-pane
            label="电器完好度"
            name="health"
            :disabled="!uploadResult?.asset_health"
          >
            <div v-if="uploadResult?.asset_health" class="drawer-tab-body">
        <el-alert
        ...
        />
            ...
            <el-empty v-else description="暂无电器健康数据" :image-size="64" />
          </el-tab-pane>

          <el-tab-pane label="运营建议" name="ops">
            <div v-loading="loadingSuggestions" class="drawer-tab-body">
        <el-alert
          v-if="!twinBuildingId"
          type="info"
          show-icon
        ...
        />
        <el-empty
          v-else-if="!loadingSuggestions && !uploadResult"
          description="请先完成上传识别"
          :image-size="56"
        />
            </div>
          </el-tab-pane>
...
<style scoped>
:deep(.auto-incident-dialog) {
  border-radius: 16px;
  overflow: hidden;
  box-shadow: 0 12px 40px rgba(24, 144, 255, 0.12), 0 4px 16px rgba(15, 23, 42, 0.06);
}

:deep(.auto-incident-dialog .el-dialog__header) {
  margin: 0;
  padding: 20px 22px 0;
  border: none;
}

:deep(.auto-incident-dialog .el-dialog__body) {
  padding: 14px 22px 8px;
}

:deep(.auto-incident-dialog .el-dialog__footer) {
  padding: 8px 22px 20px;
  border-top: 1px solid #f0f2f5;
}

.auto-incident-header {
  display: flex;
...
.auto-incident-section {
  padding: 14px 16px;
}

.auto-incident-section + .auto-incident-section {
  border-top: 1px solid #e8eef4;
}

.auto-incident-section-label {
  font-size: 12px;
  font-weight: 500;
  color: #86909c;
  margin-bottom: 8px;
...
.auto-incident-section-meta .num-font {
  color: #4e5969;
  font-weight: 600;
}

.auto-incident-section-list {
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.auto-incident-device-row {
  display: flex;
  align-items: center;
...
.auto-incident-device-row--attn .auto-incident-device-tag {
  background: #fff7e6;
  color: #d48806;
}

.auto-incident-footer {
  display: flex;
...
.auto-incident-btn-later {
  color: #86909c;
}

.auto-incident-btn-go {
  padding-left: 18px;
  padding-right: 16px;
  border-radius: 8px;
}

.auto-incident-btn-ico {
  margin-left: 4px;
}
...
.drawer-inner-card :deep(.el-card__body) {
  padding: 0;
}

:deep(.twin-drawer .el-drawer__body) {
...
.twin-forecast-chart :deep(.app-chart) {
  width: 100%;
  height: 320px;
  min-height: 320px;
}
